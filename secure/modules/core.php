...
function psec_autoban($mysqli, $type) 
{
    ...
    if ($bansvalid->num_rows <= 0) {
        ...
        $result = $mysqli->query("SELECT MAX(id) as max_id FROM `psec_bans`");
        $row = $result->fetch_assoc();
        $next_id = $row['max_id'] + 1;
        
        $log = $mysqli->query("INSERT INTO `psec_bans` (id, ip, date, time, reason, autoban) VALUES ($next_id, '$ip', '$date', '$time', '$type', '1')");
    }
}
